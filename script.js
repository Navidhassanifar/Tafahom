document.addEventListener('DOMContentLoaded', () => {
    ...
    function loadDashboard(code) {
        ...
        // بارگذاری پیشنهادات هوشمند (جدید)
...
این تفاهم‌نامه در تاریخ ${currentDate} تنظیم و امضا شد.
    ...
    // --- امکانات جدید: سیستم پیشنهاد هوشمند ---

    // دیتابیس فیک برای پیشنهادات هوشمند
    ...
    // تبدیل مقادیر رشته‌ای به عددی برای محاسبات
        ...
        const suggestions = smartFeaturesDB
            ...
            .map(c => {
                ...
                return {
                    ...
                    tier: c.tier,
                    distance: distance,
                    score: 100 - (distance * 10) // تبدیل فاصله به امتیاز (هرچه فاصله کمتر، امتیاز بیشتر)
                };
            })
            .sort((a, b) => a.distance - b.distance) // مرتب‌سازی بر اساس نزدیکی
            .slice(0, 3); // 3 پیشنهاد برتر

        return suggestions;
    }

    // بارگذاری پیشنهادات هوشمند در داشبورد
    function loadSmartSuggestions(code) {
        ...
        setTimeout(() => { // شبیه‌سازی پردازش
            ...
            // انیمیشن برای پیشنهادات هوشمند
            animateListItems('smart-suggestions-list');

            // افزودن رویداد به دکمه‌های درخواست
        ...
        }, 1000); // تاخیر 1 ثانیه برای شبیه‌سازی پردازش
    }

    // مدیریت دکمه به‌روزرسانی پیشنهادات هوشمند
