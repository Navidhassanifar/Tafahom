document.addEventListener('DOMContentLoaded', () => {
    ...
    document.querySelectorAll('a[href^="#"]').forEach(anchor => {
        anchor.addEventListener('click', function (e) {
            ...
            console.log(`کلیک روی لینک: ${targetId}`); // لاگ برای دیباگ
            ...
            if (targetId === 'home') {
                ...
                if (code) {
                    console.log(`ورود به داشبورد با کد: ${code}`); // لاگ برای دیباگ
                    loadDashboard(code);
                }
            }
        });
    ...
    contactForm.addEventListener('submit', (e) => {
        ...
        console.log('پیام ارسالی:', { name, email, message });
    ...
    form.addEventListener('submit', (e) => {
        ...
        if (regex.test(code)) {
            console.log(`استعلام شرکت با کد: ${code}`); // لاگ برای دیباگ
    ...
    successMessage.addEventListener('click', (e) => {
        if (e.target.classList.contains('dashboard-btn')) {
            console.log('دکمه ورود به داشبورد کلیک شد!'); // لاگ برای دیباگ
            const code = document.querySelector('.company-code')?.textContent;
            if (code) {
                console.log(`کد شرکت: ${code}`); // لاگ برای دیباگ
                const hero = document.querySelector('.hero');
                const sections = document.querySelectorAll('.section');
                const dashboard = document.getElementById('dashboard');

                // مخفی کردن همه بخش‌ها به جز داشبورد
                hero.classList.add('hidden');
                sections.forEach(section => {
                ...
                // نمایش داشبورد
                dashboard.classList.remove('hidden');
                dashboard.style.display = 'block'; // اطمینان از نمایش داشبورد
                console.log('داشبورد باید نمایش داده شود!'); // لاگ برای دیباگ
                loadDashboard(code);
                document.getElementById('dashboard-link').style.display = 'inline';
                document.getElementById('logout-btn').style.display = 'inline';
                document.querySelectorAll('a[href^="#"]:not(#dashboard-link):not(#logout-btn)').forEach(link => {
                ...
                dashboard.scrollIntoView({ behavior: 'smooth' });
                successMessage.style.display = 'none'; // مخفی کردن پیام موفقیت
                modal.style.display = 'none'; // بستن مودال استعلام شرکت
            } else {
                console.error('کد شرکت پیدا نشد!'); // لاگ برای دیباگ
            }
        }
    });
    ...
    function loadDashboard(code) {
        console.log('تابع loadDashboard اجرا شد!'); // لاگ برای دیباگ
        try {
            const companyData = JSON.parse(localStorage.getItem('companyData_' + code)) || {};
            const subscription = JSON.parse(localStorage.getItem('subscription_' + code)) || {};

            console.log('داده‌های شرکت:', companyData); // لاگ برای دیباگ
            console.log('داده‌های اشتراک:', subscription); // لاگ برای دیباگ
            ...
            if (welcomeMessage) {
                ...
                console.error('المان welcome-message پیدا نشد!'); // لاگ برای دیباگ
            }
            ...
            console.log('بارگذاری داشبورد با موفقیت انجام شد!'); // لاگ برای دیباگ
        } catch (error) {
            console.error('خطا در بارگذاری داشبورد:', error); // لاگ برای دیباگ
        }
    }

    // تابع بارگذاری اعلان‌ها
    function loadNotifications(code, subscription) {
        console.log('تابع loadNotifications اجرا شد!'); // لاگ برای دیباگ
    ...
    function loadCollaborations(code) {
        console.log('تابع loadCollaborations اجرا شد!'); // لاگ برای دیباگ
        ...
        collaborationData.forEach(collab => {
            ...
            row.innerHTML = `
                ...
                <td><button class="btn cancel-btn" data-id="${collab.id}" ${
                    collab.status !== 'active' ? 'disabled' : ''
                }>لغو همکاری</button></td>
            `;
    ...
    const collabModal = document.getElementById('collaboration-details-modal');
    const collabCloseBtn = document.querySelector('.collaboration-close-btn');
